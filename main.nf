...
def helpMessage() {
    ...
    nextflow run nf-core/evoverse -r <VERSION> -profile <PROFILE> --samples <INPUT CSV> --publish_dir ./results --tools <TOOLS>

    Mandatory arguments:
      --samples [file]                Path to input csv (must be surrounded with quotes)
      -profile [str]                  Configuration profile to use. Can use multiple (comma separated)
                                      Available: conda, docker, singularity, test, awsbatch, <institute> and more

    Options:
       --mode [str]                   Modality of pipeline run, either "single_sample" or "multi_sample"

    References                        If not specified in the configuration file or you wish to overwrite any of the references
      --ref_genome [file]             Path to fasta reference
...
workflow.onComplete {
    ...
    def subject = "[nf-core/evoverse] Successful: $workflow.runName"
    if (!workflow.success) {
        subject = "[nf-core/evoverse] FAILED: $workflow.runName"
    }
    def email_fields = [:]
 ...
 // Check if we are only sending emails on failure
    ...
    if (email_address) {
        try {
            ...
            log.info "[nf-core/evoverse] Sent summary e-mail to $email_address (sendmail)"
        } catch (all) {
            // Catch failures and try with plaintext
            [ 'mail', '-s', subject, email_address ].execute() << email_txt
            log.info "[nf-core/evoverse] Sent summary e-mail to $email_address (mail)"
        }
    }
    ...
    if (workflow.success) {
        log.info "-${c_purple}[nf-core/evoverse]${c_green} Pipeline completed successfully${c_reset}-"
    } else {
        checkHostname()
        log.info "-${c_purple}[nf-core/evoverse]${c_red} Pipeline completed with errors${c_reset}-"
    }
...
def nfcoreHeader() {
    ...
    ${c_blue}  | \\| |       \\__, \\__/ |  \\ |___     ${c_green}\\`-._,-`-,${c_reset}
                                            ${c_green}`._,._,\'${c_reset}
    ${c_purple}  nf-core/evoverse v${workflow.manifest.version}${c_reset}
    -${c_dim}--------------------------------------------------${c_reset}-
    """.stripIndent()
}
